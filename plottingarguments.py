...
def plottingarguments(tipo=1,cb='no',Ars=60,N=10,rel=5,l=0,bins=1,chosen='indeciso'):    
    '''This function was used for saving variables for making different kinds of plots involving arguments. The first type can be used for
    making the histograms of final arguments of Figure 2 b) and c). The second type can be used for making the argument evolutions 
    presented in Figure 3. The plotting functions are below.
    ...
    from newrunargsim import checkingargumentsevolution #import the proper simulation function
    
    if tipo==1: #for making final arguments histogram (Figure 2 (b) and (c))
        
        import numpy as np #useful import
        ...
        while count<100: #choose how many copies you want to use in the ensamble.
            ...
            b=c[-2]
            k=len(b)-1 #we only keep the arguments of the last step
            b[k]=restructuringargumentvalues(b,k) #we change the structure for easy access.
            opinions=[] #initializations
            ...
            if chosen==chosentype:
                ...
                a+=systemargs #add it to the final list
                count+=1 #elevate count, the variable used by the while loop.
                          
        return a #return final list   

    
    if tipo==2: #for making argument evolution plots (Figure 3 first column)
...
def finalargumentdistribution(cb='no',chosen='consenso2',N=10,Nen=100,bins=1):
    '''Having run plottingarguments and saved the results for type 1 (this should be done after running it), we can now make the 
    proper histograms of final arguments.
    "chosen" is the type of final state, as described in the function above.    
    "cb" is also the same as before
    "N" is the number of agents.
    "Nen" is the number of copies of the ensamble.'''
    
    from loading import loadinggeneric #this function is used for loading the results of plottingarguments
    if cb=='no': #case without confirmation bias
        a=loadinggeneric(chosen+'sincb','D:\\Doctorado\\Simulation_Functions\\')
    else: #case with confirmation bias
        a=loadinggeneric(chosen+'concb','C:\\Users\\Fede Barrera\\Desktop\\Simulation_Functions\\')    
    if N==100: #we only used N=10 or N=100, so this command is enough to choose between the two.
        a=loadinggeneric(chosen+'sincb100','D:\\Doctorado\\Simulation_Functions\\') 
    
    #plotting commands               
    import pylab as py
    py.figure()
    py.hist(a,(60/bins)+1) #this makes the histogram
    #axes labels
...
def plottingmanyevolutionsbins1(chosen='indeciso',cb='no',N=10,Ars=60,rel=6):
        '''Having run plottingarguments and saved the results for type 2 (this should be done after running it), we can now make the 
        proper plots of evolutions of arguments. This was optimized for bins of size 1; making it for larger bins requires some tweaking.
        We will not be making a more general function, but we added comments in the lines of code that should be changed for making it.
        "chosen" is the type of final state, as described in the function above.
        "cb" is also the same as before
        "N" is the number of agents.
